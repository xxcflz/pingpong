...
async function resetServer() {
  ...
  await sleep(1000);
  const s = await lobby();
  console.log(`   [reset] lobby=${s.phase} room=${s.roomPhase} disc=${s.disconnectedUsers?.length}`);
}

async function lobby() { return (await fetch(`${SERVER}/api/_debug/lobby`)).json(); }
...
async function createCtx(id, name) {
  ...
  await page.waitForFunction(() => !!window.__orchestrator && window.__orchestrator.phase !== "idle", { timeout: 20_000 });
  await sleep(800);

  await page.evaluate(() => {
    window.__hbInterval = setInterval(() => window.__orchestrator?.socket?.emit("lobby_heartbeat"), 3000);
    window.__matchEndEvents = []; window.__pauseEvents = []; window.__resumeEvents = [];
    window.__stateSnaps = []; window.__serverErrors = []; window.__cspViolations = [];
    ...
    if (s) {
      ...
      s.on("stateSnapshot", d => { if (window.__stateSnaps.length < 600) window.__stateSnaps.push({ x: d.ball.pos.x, y: d.ball.pos.y }); });
...
async function cleanup(tc) {
  if (!tc) return;
  try { await tc.page.evaluate(() => clearInterval(window.__hbInterval)); await tc.ctx.close(); } catch {}
}

async function ready(tc) {
  return tc.page.evaluate(() => { window.__orchestrator?.socket?.emit("readyToggle"); return true; });
}
...
function ping(tc, x = 360) {
  const id = setInterval(() => {
    tc.page.evaluate(px => window.__orchestrator?.socket?.emit("paddleInput", { seq: Date.now(), x: px }), x).catch(() => {});
  }, 600);
...
const shot = (tc, name) => tc.page.screenshot({ path: join(EVIDENCE, `${name}.png`), fullPage: true });
...
async function scenario1() {
  ...
  try {
    ...
    const cReady = await C.page.evaluate(() => !!window.__pong_scene?.getLayers()?.uiLayer?.getChildByLabel?.("readyOverlay", true));

    const playOk = await waitPhase(A, "playing", 15_000);
    if (playOk) { await sleep(1500); await shot(A, "s1-playing"); }
...
async function scenario2() {
  ...
  try {
    ...
    await sleep(3000);
    const l = await lobby();
    const lfe = await Promise.all([A, B, C].map(tc =>
      tc.page.evaluate(() => (window.__serverErrors ?? []).filter(e => e.code === "LOBBY_FULL").length)));
  ...
  } catch (e) { record("Lobby race", false, e.message); }
  finally { await cleanup(A); await cleanup(B); await cleanup(C); }
}
...
async function scenario3() {
  ...
  try {
    await ready(A); await ready(B);
    const playOk = await waitPhase(A, "playing", 15_000);
    if (!playOk) { record("Disconnect → forfeit_dc", false, "no playing"); return; }
    ...
    record("Disconnect → forfeit_dc", pass, `reason=${endData?.end_reason} winner=${endData?.winnerSlot}`);
  } catch (e) { record("Disconnect → forfeit_dc", false, e.message); }
  finally { await cleanup(A); await cleanup(B); }
}
...
async function scenario4() {
  ...
  try {
    await ready(A); await ready(B);
    const playOk = await waitPhase(A, "playing", 15_000);
    if (!playOk) { record("AFK forfeit", false, "no playing"); return; }
    ...
    record("AFK forfeit (30s)", pass, `reason=${endData?.end_reason} winner=${endData?.winnerSlot}`);
  } catch (e) { record("AFK forfeit", false, e.message); }
  finally { await cleanup(A); await cleanup(B); }
}
...
async function scenario5() {
  ...
  try {
    await ready(A); await ready(B);
    const playOk = await waitPhase(A, "playing", 15_000);
    if (!playOk) { record("Reconnect → resume", false, "no playing"); return; }
    ...
    await B.page.evaluate(() => {
      window.__pn = 0; window.__rn = 0;
      const s = window.__orchestrator?.socket;
      if (s) { s.on("pause", () => window.__pn++); s.on("resume", () => window.__rn++); }
    });
    ...
    console.log(`   Lobby: phase=${lBefore.phase} disc=${JSON.stringify(lBefore.disconnectedUsers)}`);
    ...
    await cleanup(A2);
  } catch (e) { record("Reconnect → resume", false, e.message); }
  finally { await cleanup(A); await cleanup(B); }
}
...
async function scenario6() {
  ...
  try {
    ...
    await sleep(3000);
    await A.page.evaluate(() => { window.__stateSnaps = []; });
    await sleep(500);
    ...
    const after = await A.page.evaluate(() => { const s = window.__stateSnaps; return s.length ? s[s.length - 1] : null; });
    ...
    record("Tab-hidden resync", pass, `phase=${p} noReset=${dist > 50} snaps=${snaps} dist=${dist.toFixed(0)}`);
  } catch (e) { record("Tab-hidden resync", false, e.message); }
  finally { await cleanup(A); await cleanup(B); }
}
...
async function scenario7() {
  ...
  try {
    ...
    const v = await Promise.all([A, B, C].map(tc => tc.page.evaluate(() => window.__cspViolations ?? [])));
    const allV = v.flat();
    await shot(A, "s7-csp");
    const cspE = [...A.errors, ...B.errors, ...C.errors].filter(e => e.includes("Security Policy") || e.includes("Refused to"));
    const pass = allV.length === 0 && cspE.length === 0;
    record("CSP zero violations", pass, `violations=${allV.length} cspErrors=${cspE.length}`);
  } catch (e) { record("CSP zero violations", false, e.message); }
  finally { await cleanup(A); await cleanup(B); await cleanup(C); }
}
...
async function main() {
  ...
  try { const r = await fetch(`${SERVER}/api/_debug/lobby`); if (!r.ok) throw new Error(); console.log("✓ Server OK\n"); }
  catch { console.error("✗ Server down"); process.exit(1); }

  browser = await chromium.launch({ headless: true });

  await resetServer(); await scenario1();
  await resetServer(); await scenario2();
  await scenario3();
  await scenario4();
  await scenario5();
  await scenario6();
  ...
  const edge = results.filter(r => ["Disconnect", "AFK", "Reconnect", "Tab-hidden"].some(n => r.name.includes(n)));
  const edgePass = edge.filter(r => r.pass).length;
  const csp = results.find(r => r.name.includes("CSP"));
  const cross = results.filter(r => ["Full match", "Lobby race", "Reconnect"].some(n => r.name.includes(n)));
...
`;
  ...
  writeFileSync(join(EVIDENCE, "results.json"), JSON.stringify({ results, summary, passed, total }, null, 2));
  console.log(`\n📄 ${EVIDENCE}/report.md`);
  process.exit(0);
}
