...
async function run() {
  ...
  const inspect = await page.evaluate(() => ({
    ...
    paddleBottom: !!window.__pong_layers?.playLayer?.children.find(
      (c) => c.label === 'paddleBottom',
    ),
  ...
  writeFileSync(
    `${EVIDENCE}/task-13-fps.txt`,
    `Mean frame time: ${mean.toFixed(2)}ms\nSamples: ${JSON.stringify(samples)}\n`,
  );
  const s2 = mean < 18;
  ...
  try {
    ...
    const size = execSync('cat packages/client/dist/assets/*.js | gzip -9 | wc -c', {
      encoding: 'utf8',
    }).trim();
    writeFileSync(`${EVIDENCE}/task-13-bundle.txt`, `${size} bytes gzipped\n`);
    const s3 = Number.parseInt(size) < 512000;
  ...
  console.log(
    `Canvas ratio: ${ratio.toFixed(4)} (expected ~${expected.toFixed(4)}) — ${s4 ? 'PASS ✓' : 'FAIL ✗'}`,
  );
  if (!s4) allPass = false;
