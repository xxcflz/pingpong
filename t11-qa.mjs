...
function waitForHealth(maxRetries = 15) {
  return new Promise(async (res, rej) => {
    ...
    rej(new Error("Health check failed"));
  });
}
...
function connect(claim, testUserId, ns = "") {
  return new Promise((res, rej) => {
    ...
    socket.on("connect", () => { clearTimeout(timer); res(socket); });
    socket.on("connect_error", (err) => { clearTimeout(timer); rej(err); });
  });
}
...
async function run() {
  ...
  const p1 = await connect("top", "player_top", "/").catch(e => { console.error("P1 fail:", e.message); throw e; });
  const p2 = await connect("bottom", "player_bot", "/").catch(e => { console.error("P2 fail:", e.message); throw e; });
  await sleep(2000);
  ...
  {
    const spec = await connect(null, "spectator_1", "/spectate")
      .catch(e => { console.error("Spec1 fail:", e.message); throw e; });
    ...
    writeFileSync(`${EVIDENCE}/task-11-spectator-rate.txt`,
  ...
  console.log("\n=== Scenario 2: Spectator payload size ===");
  {
    ...
    const pass = med < 500;
    console.log(`  Samples: ${sizes.length}, Median: ${med} bytes, Min: ${Math.min(...sizes)}, Max: ${Math.max(...sizes)} — ${pass ? "PASS" : "FAIL"}`);
    writeFileSync(`${EVIDENCE}/task-11-spectator-size.txt`,
  ...
  console.log("\n=== Scenario 3: Player cannot spectate ===");
  {
    ...
    console.log(`  Error: ${errorReceived?.code ?? "none"}, Disconnected: ${disconnected} — ${pass ? "PASS" : "FAIL"}`);
    writeFileSync(`${EVIDENCE}/task-11-no-double-role.txt`,
